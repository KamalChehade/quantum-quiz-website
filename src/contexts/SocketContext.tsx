...
export const SocketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [questionLeaderboard, setQuestionLeaderboard] = useState<QuestionLeaderboardRow[]>([]);
  ...
  const [gameStatus, setGameStatus] = useState<'idle' | 'active' | 'ended'>('idle');
  const [adminOnline, setAdminOnline] = useState(false);
  const [myAnswerSubmitted, setMyAnswerSubmitted] = useState(false);
  const [myAnswerForQuestionId, setMyAnswerForQuestionId] = useState<number | null>(null);
  ...
  const initializeSocket = (token?: string) => {
      ...
      try {
        ...
        if (Array.isArray(rawChoices)) {
          if (rawChoices.length > 0 && typeof rawChoices[0] === 'string') {
            ...
            choices = rawChoices.map((c: string, i: number) => ({ key: letters[i] ?? String(i + 1), text: String(c) }));
          } else {
          ...
          choices = Object.keys(rawChoices).map((k) => ({ key: k, text: String(rawChoices[k]) }));
        } else {
    ...
    try {
      if (typeof window !== 'undefined' && !(window as any).__socketPathPatched) {
        (window as any).__socketPathPatched = true;
        const targetPath = (import.meta.env.VITE_SOCKET_PATH as string | undefined) || '/qa-api/socket.io';
        ...
        if (window.fetch) {
          ...
          window.fetch = (input: URL | RequestInfo, init?: RequestInit) => {
            try {
              ...
              if (url && url.startsWith('/') && url.indexOf('/socket.io') === 0) {
                const newUrl = (import.meta.env.VITE_SOCKET_PATH as string | undefined) || '/qa-api/socket.io';
        ...
        console.debug('[Socket] runtime rewrite installed to map /socket.io ->', targetPath);
      }
    } catch (e) {
      /* ignore patch errors */
    }
    const socketBase =
      ...
      'http://localhost:3000';
    const socketPath = (import.meta.env.VITE_SOCKET_PATH as string | undefined) || '/qa-api/socket.io';
    ...
    if (token) {
      ...
      const outToken = typeof token === 'string' && token.startsWith('Bearer ') ? token : `Bearer ${token}`;
      socketOptions.auth = { token: outToken };
    }

    console.debug('[Socket] connecting', { socketBase, socketPath, socketOptions });
    try {
    ...
    newSocket.on('connect', () => {
      setIsConnected(true);
      try {
        console.debug('[Socket] connected', { id: newSocket.id, url: socketBase + socketPath });
      } catch (e) {
      ...
      try {
        ...
        newSocket.timeout(2000).emit('is_admin_present', undefined, (...cbArgs: any[]) => {
          try {
            ...
            const present = resp?.present ?? (typeof resp === 'boolean' ? resp : false);
      ...
      try {
        ...
        if (raw) {
          ...
          if (saved && (saved.name || saved.phone)) {
            ...
            newSocket.emit('join_quiz', { name: saved.name, phone: saved.phone }, (ack: any) => {
              try {
                const ok = ack?.ok ?? true;
                if (ok) {
                  if (ack?.participant) {
                    setParticipant(ack.participant);
                    localStorage.setItem('participant', JSON.stringify(ack.participant));
                  }
    ...
    newSocket.on('resume_question', (payload: any) => {
      ...
      if (activeQuestionIdRef.current !== null && activeQuestionIdRef.current !== qid && lastQuestionEventTypeRef.current === 'started') {
        console.debug('[Socket] resume_question -> stale; ignoring', qid, 'active:', activeQuestionIdRef.current);
        return;
      }
      ...
      setCurrentQuestion(q);
      const answered = !!(payload?.hasAnswered || payload?.answeredAt || payload?.answered_at);
      if (answered) {
        setMyAnswerSubmitted(true);
        setMyAnswerForQuestionId(qid);
        setMySelectedAnswer(payload?.selectedAnswer ?? payload?.selected_answer ?? null);
      } else {
        setMyAnswerSubmitted(false);
        setMyAnswerForQuestionId(null);
        setMySelectedAnswer(null);
    ...
    newSocket.on('question_resumed', (payload: any) => {
      ...
      if (!qid) return;
      // If a start already set a different active id, ignore this resume as stale
      if (activeQuestionIdRef.current !== null && activeQuestionIdRef.current !== Number(qid) && lastQuestionEventTypeRef.current === 'started') {
        console.debug('[Socket] question_resumed -> stale; ignoring', qid, 'active:', activeQuestionIdRef.current);
        return;
      }
      // Accept resume
    ...
    newSocket.on('question_leaderboard', (payload: any) => {
      ...
      const mapped: QuestionLeaderboardRow[] = (lb || []).map((p: any, i: number) => ({
        id: p.id ?? undefined,
    ...
    newSocket.on('admin_presence', (payload: any) => {
      try {
        const present = payload?.present ?? (typeof payload === 'boolean' ? payload : !!payload);
    ...
    newSocket.on('game_leaderboard', (payload: any) => {
      const lb = payload?.leaderboard ?? payload ?? [];
      const mapped: GameLeaderboardEntry[] = (lb || []).map((p: any, i: number) => ({
        ...
        totalQuestionsAnswered: p.totalQuestionsAnswered ?? p.answeredCount ?? undefined,
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (cancelled) return;
      try {
        socket.timeout(2000).emit('is_admin_present', undefined, (...cbArgs: any[]) => {
          try {
            let resp = cbArgs.length === 1 ? cbArgs[0] : cbArgs[1] ?? cbArgs[0];
            const present = resp?.present ?? (typeof resp === 'boolean' ? resp : false);
            setAdminOnline(Boolean(present));
  ...
  const submitAnswer = (questionId: number, selectedAnswer: string) => {
    return new Promise<any>((resolve) => {
      ...
      try {
        socket.emit('submit_answer', { questionId, selectedAnswer }, (ack: any) => {
          if (cleared) return;
          cleared = true;
  ...
  const value: SocketContextType = {
    ...
    setGameLeaderboardLocal: (g: GameLeaderboardEntry[]) => setGameLeaderboard(g),
    setQuestionLeaderboardLocal: (q: QuestionLeaderboardRow[]) => setQuestionLeaderboard(q),
